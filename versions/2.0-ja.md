...
<a name="swaggerSecurityDefinitions"></a>securityDefinitions | [Security Definitions Object](#securityDefinitionsObject) | 仕様全体で使われる セキュリティスキーマ定義。
<a name="swaggerSecurity"></a>security | [[Security Requirement Object](#securityRequirementObject)] | API全体に適用されるセキュリティスキーマの宣言。リストの値は、選択的に使用できるスキーマを表す（つまり、セキュリティ要件のOR条件になる）。個別の操作はこの定義を上書きできる。
...
<a name="operationSecurity"></a>security | [[Security Requirement Object](#securityRequirementObject)] | この操作に適用されるセキュリティスキーマ宣言。値のリストは、選択的に使用できるセキュリティスキーマを表す（つまり、セキュリティ要件のOR条件となる）。この定義は、最上位で宣言された [`security`](#swaggerSecurity) を上書きする。最上位の security 宣言を削除するために、空の配列を使用できる。
...
<a name="responseSchema"></a>schema | [Schema Object](#schemaObject) | レスポンスの構造の定義。プリミティブ型・配列・オブジェクトが使用できる。 項目が存在しない場合、レスポンスとしてコンテンツが返らないことを示す。[Schema Object](#schemaObject) の拡張として、最上位の `type` の値は `"file"` でもよい(MAY)。この場合、関係する `produces` のMIMEタイプを伴うべきである(SHOULD)。
<a name="responseHeaders"></a>headers | [Headers Object](#headersObject) | レスポンスとともに送信されるヘッダのリスト。
<a name="responseExamples"></a>examples | [Example Object](#exampleObject) | レスポンスメッセージの例。
...
項目パターン | 型 | 説明
---|:---:|---
<a name="headersName"></a>{name} | [Header Object](#headerObject) | ヘッダの名前に対応するプロパティ名。値はヘッダの型を記述する。
...
項目パターン | 型 | 説明
---|:---:|---
<a name="exampleMimeType"></a>{[mime type](#mimeTypes)} | Any | プロパティの名前は、操作の `produces` の（明示的な、または継承した）値のいずれか一つでなければならない(MUST)。値は、レスポンスがどのように見えるかの例であるべきです（SHOULD）。
...
<a name="headerFormat"></a>format | `string` | 上述の [`type`](#parameterType) を拡張する形式。詳細は [Data Type Formats](#dataTypeFormat) を参照。
<a name="headerItems"></a>items | [Items Object](#itemsObject) | **パラメータの [`type`](#parameterType) が "array" の場合必須。** 配列内の要素の型を記述する。
...
[Operation Object](#operationObject) で使用する単一のタグにメタデータを追加できる。タグにTag オブジェクトを持たせることは必須ではない。
...
<a name="tagExternalDocs"></a>externalDocs | [External Documentation Object](#externalDocumentationObject) | タグに関する追加の外部文書。
...
- format (詳細は [Data Type Formats](#dataTypeFormat) を参照)
- title
...
以下のプロパティは JSON Schema 定義に由来しているが、それらの定義は Swagger 仕様に合わせて調整されている。これらの定義は、JSON Schema の定義と等しい。元の定義がJSON Schema の定義を参照している場合にのみ、[Schema Object](#schemaObject) の定義が代わりに使用される。
- items
- allOf
- properties
...
<a name="schemaXml"></a>xml | [XML Object](#xmlObject) | スキーマのプロパティでのみ仕様できる(MAY)。最上位のスキーマにはなんの効果もない。このプロパティに追加のメタデータをXML形式で追加する。
<a name="schemaExternalDocs"></a>externalDocs | [External Documentation Object](#externalDocumentationObject) | このスキーマに関する追加の外部文書。
...
[xml](#schemaXml)プロパティは、JSON定義をXMLに翻訳する時の追加の定義を持たせられる。[XML Object](#xmlObject) は、使用可能なオプションに関する追加情報を含む。
...
XMLオブジェクト定義の例は、[Schema Object](#schemaObject) のプロパティ定義の中に、XML表現の例として含まれている。
...
項目パターン | 型 | 説明
---|:---:|---
<a name="definitionsName"></a>{name} | [Schema Object](#schemaObject) | 単一の定義。スキーマの定義と名前を紐付ける。
...
項目パターン | 型 | 説明
---|:---:|---
<a name="pdName"></a>{name} | [Parameter Object](#parameterObject) | 単一のパラメータ定義。パラメータの定義と名前を紐付ける。
...
項目パターン | 型 | 説明
---|:---:|---
<a name="rdName"></a>{name} | [Response Object](#responseObject) | 単一のレスポンス定義。レスポンス定義と名前を紐付ける。
...
項目パターン | 型 | 説明
---|:---:|---
<a name="sdName"></a>{name} | [Security Scheme Object](#securitySchemeObject) | 単一のセキュリティスキーマ定義。名前とスキーマ定義を紐付ける。
...
<a name="securitySchemeScopes"></a>scopes | [Scopes Object](#scopesObject) | `oauth2` | **必須。** OAuth2 セキュリティスキーマにおける、利用可能なスコープのリスト。
...
各プロパティの名前は、[Security Definitions](#securityDefinitionsObject) で宣言されたセキュリティスキーマに対応していなければならない(MUST)。

##### パターン化項目

項目パターン | 型 | 説明
---|:---:|---
<a name="securityRequirementsName"></a>{name} | [`string`] | 各プロパティの名前は、[Security Definitions](#securityDefinitionsObject) で宣言されたセキュリティスキーマに対応していなければならない(MUST)。セキュリティスキーマが `"oauth2"` のいずれかの場合、値は実行に必要なスコープの名前のリストである。その他のセキュリティスキーマでは、配列は空でなければならない(MUST)。
...
1. [Paths Object](#pathsObject) が空になることがある。これは直観に反するだろうが、ビューアに場所としては正しいがアクセスできる文書がないことを伝えるだろう。認証に関する追加情報があるであろう [Info Object](#infoObject) には、この状態でもアクセスできる。
2. [Path Item Object](#pathItemObject) が空になることがある。この場合、ビューアはパスが存在するが、操作やパラメータは見られないことが分かる。これはパス自身が[Paths Object](#pathsObject)から隠され、パスの存在自体がわからなくなるのとは異なる。文書提供者は、これを利用してビューアが見られる範囲をきめ細かく制御できる。
