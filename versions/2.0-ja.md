...
* Form - `application/x-www-form-urlencoded` または `multipart/form-data` がリクエストのコンテンツタイプ（Swaggerの定義では、操作の [`consumes`](#operationConsumes) プロパティにあたる）になっている時のHTTPリクエストのペイロードを記述するために使用する。ファイルを送信する場合に使うことができる唯一のパラメータ種類である。フォームパラメータもペイロードに入るため、body と form は同じ操作に同時に存在することはできない。フォームパラメータは、使用するContent-typeによって異なる形式を持つ（詳細は http://www.w3.org/TR/html401/interact/forms.html#h-17.13.4 を参照）。
