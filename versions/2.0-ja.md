...
<a name="swaggerBasePath"></a>basePath | `string` | APIが提供される基底となる、[`host`](#swaggerHost) への相対的なパス。省略された場合、APIは `host` の直下で提供される。値はスラッシュ(`/`)で始めなければならない(MUST)。 `basePath` では [パステンプレート](#pathTemplating)は使えない。
