...
メディアタイプの定義は複数のリソースにまたがっている。
メディアタイプの定義は、[RFC 6838](http://tools.ietf.org/html/rfc6838) に準拠した方がよい(SHOULD)。

メディアタイプ定義の例は以下の通りである。
```
...
HTTP ステータスコードは実行した操作の状態を示すために使う。
利用できるステータスコードは [RFC 7231](http://tools.ietf.org/html/rfc7231#section-6) で定義され、登録されたステータスコードは [IANA Status Code Registry](http://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml) に列挙されている。
...
The schema exposes two types of fields: 固定項目, which have a declared name, and Patterned fields, which declare a regex pattern for the field name.
...
OASにおけるプリミティブデータ型は、[JSON Schema Specification Wright Draft 00](https://tools.ietf.org/html/draft-wright-json-schema-00#section-4.2) でサポートされている型にもとづいている。
Note that `integer` as a type is also supported and is defined as a JSON number without a fraction or exponent part.
`null` is not supported as a type (see [`nullable`](#schemaNullable) for an alternative solution).
モデルは JSON Schema Specification Wright Draft 00 のサブセットである [Schema Object](#schemaObject) を使って記述される。

<a name="dataTypeFormat"></a> プリミティブ型は任意の修飾プロパティ `format` がある。OAS は、使用するデータ型を精密に定義するために、幾つかのよく知られた形式を使用する。
しかし、文書化の必要に応えるために `format` プロパティは `string`の値を自由入力するプロパティであり、どのような値でも持つことができる。
`"email"`や`"uuid"`といった形式などは、この仕様で定義されていなくても使用することができる(MAY)。`format` プロパティがない型は、JSON Schema におけるその定義に従う。Tools that do not recognize a specific `format` MAY default back to the `type` alone, as if the `format` is not specified.

OAS が定める format は以下の通りである。

一般的な名前 | [`type`](#dataTypes) | [`format`](#dataTypeFormat) | コメント
----------- | ------ | -------- | --------
...
password | `string` | `password` | 入力を隠すUIへのヒント
...
In the following description, if a field is not explicitly **必須** or described with a MUST or SHALL, it can be considered OPTIONAL.

#### <a name="oasObject"></a>OpenAPI オブジェクト

これは、[OpenAPI document](#oasDocument)における最上位のドキュメントオブジェクトである。

##### 固定項目

項目名 | 型 | 説明
---|:---:|---
<a name="oasVersion"></a>openapi | `string` | **必須**。 This string MUST be the [semantic version number](https://semver.org/spec/v2.0.0.html) of the [OpenAPI Specification version](#versions) that the OpenAPI document uses. The `openapi` field SHOULD be used by tooling specifications and clients to interpret the OpenAPI document. This is *not* related to the API [`info.version`](#infoVersion) string.
<a name="oasInfo"></a>info | [Info Object](#infoObject) | **必須**。 Provides metadata about the API. The metadata MAY be used by tooling as required.
...
<a name="oasPaths"></a>paths | [Paths Object](#pathsObject) | **必須**。 The available paths and operations for the API.
...
このオブジェクトは [Specification Extensions](#specificationExtensions) によって拡張できる(MAY)。

#### <a name="infoObject"></a>Info Object

APIに関するメタデータを提供するオブジェクト。
メタデータは必要に応じてクライアントが利用したり、利便性のために編集・文書生成ツールで表示したりできる(MAY)。
...
<a name="infoDescription"></a>description | `string` | アプリケーションの短い説明。リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
<a name="infoVersion"></a>version | `string` | **必須**。 The version of the OpenAPI document (which is distinct from the [OpenAPI Specification version](#oasVersion) or the API implementation version).
...
項目名 | 型 | 説明
---|:---:|---
<a name="serverUrl"></a>url | `string` | **必須**。 A URL to the target host.  This URL supports Server Variables and MAY be relative, to indicate that the host location is relative to the location where the OpenAPI document is being served. Variable substitutions will be made when a variable is named in `{`brackets`}`.
<a name="serverDescription"></a>description | `string` | An optional string describing the host designated by the URL. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
<a name="serverVariableDefault"></a>default | `string` |  **必須**。 The default value to use for substitution, and to send, if an alternate value is _not_ supplied. Unlike the [Schema Object's](#schemaObject) `default`, this value MUST be provided by the consumer.
<a name="serverVariableDescription"></a>description | `string` | An optional description for the server variable. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
All the 固定項目 declared above are objects that MUST use keys that match the regular expression: `^[a-zA-Z0-9\.\-_]+$`.
...
<a name="pathItemDescription"></a>description | `string` | An optional, string description, intended to apply to all operations in this path. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
<a name="operationDescription"></a>description | `string` | A verbose explanation of the operation behavior. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
<a name="operationResponses"></a>responses | [Responses Object](#responsesObject) | **必須**。 The list of possible responses as they are returned from executing this operation.
...
項目名 | 型 | 説明
---|:---:|---
<a name="externalDocDescription"></a>description | `string` | A short description of the target documentation. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
<a name="externalDocUrl"></a>url | `string` | **必須**。 The URL for the target documentation. Value MUST be in the format of a URL.
...
項目名 | 型 | 説明
---|:---:|---
<a name="parameterName"></a>name | `string` | **必須**。 The name of the parameter. Parameter names are *case sensitive*. <ul><li>If [`in`](#parameterIn) is `"path"`, the `name` field MUST correspond to the associated path segment from the [path](#pathsPath) field in the [Paths Object](#pathsObject). See [Path Templating](#pathTemplating) for further information.<li>If [`in`](#parameterIn) is `"header"` and the `name` field is `"Accept"`, `"Content-Type"` or `"Authorization"`, the parameter definition SHALL be ignored.<li>For all other cases, the `name` corresponds to the parameter name used by the [`in`](#parameterIn) property.</ul>
<a name="parameterIn"></a>in | `string` | **必須**。 The location of the parameter. Possible values are "query", "header", "path" or "cookie".
<a name="parameterDescription"></a>description | `string` | A brief description of the parameter. This could contain examples of use.  リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
<a name="parameterRequired"></a>required | `boolean` | Determines whether this parameter is mandatory. If the [parameter location](#parameterIn) is "path", this property is **必須** and its value MUST be `true`. Otherwise, the property MAY be included and its default value is `false`.
...
項目名 | 型 | 説明
---|:---:|---
<a name="requestBodyDescription"></a>description | `string` | A brief description of the request body. This could contain examples of use.  リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
<a name="requestBodyContent"></a>content | Map[`string`, [Media Type Object](#mediaTypeObject)] | **必須**。 The content of the request body. The key is a media type or [media type range](https://tools.ietf.org/html/rfc7231#appendix-D) and the value describes it.  For requests that match multiple keys, only the most specific key is applicable. e.g. text/plain overrides text/*
...
When passing in `multipart` types, boundaries MAY be used to separate sections of the content being transferred â€” thus, the following default `Content-Type`s are defined for `multipart`:
...
項目名 | 型 | 説明
---|:---:|---
<a name="responseDescription"></a>description | `string` | **必須**。 A short description of the response. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
<a name="exampleDescription"></a>description | `string` | Long description for the example. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
<a name="linkDescription"></a>description  | `string` | A description of the link. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
項目名 | 型 | 説明
---|:---:|---
<a name="tagName"></a>name | `string` | **必須**。 The name of the tag.
<a name="tagDescription"></a>description | `string` | A short description for the tag. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
項目名 | 型 | 説明
---|:---:|---
<a name="referenceRef"></a>$ref | `string` | **必須**。 The reference string.
...
- description - リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
...
項目名 | 型 | 説明
---|:---:|---
<a name="propertyName"></a>propertyName | `string` | **必須**。 The name of the property in the payload that will hold the discriminator value.
...
---|:---:|---|---
<a name="securitySchemeType"></a>type | `string` | Any | **必須**。 The type of the security scheme. Valid values are `"apiKey"`, `"http"`, `"oauth2"`, `"openIdConnect"`.
<a name="securitySchemeDescription"></a>description | `string` | Any | A short description for security scheme. リッチテキスト表現として、[CommonMark syntax](http://spec.commonmark.org/) を使用できる(MAY)。
<a name="securitySchemeName"></a>name | `string` | `apiKey` | **必須**。 The name of the header, query or cookie parameter to be used.
<a name="securitySchemeIn"></a>in | `string` | `apiKey` | **必須**。 The location of the API key. Valid values are `"query"`, `"header"` or `"cookie"`.
<a name="securitySchemeScheme"></a>scheme | `string` | `http` | **必須**。 The name of the HTTP Authorization scheme to be used in the [Authorization header as defined in RFC7235](https://tools.ietf.org/html/rfc7235#section-5.1).
...
<a name="securitySchemeFlows"></a>flows | [OAuth Flows Object](#oauthFlowsObject) | `oauth2` | **必須**。 An object containing configuration information for the flow types supported.
<a name="securitySchemeOpenIdConnectUrl"></a>openIdConnectUrl | `string` | `openIdConnect` | **必須**。 OpenId Connect URL to discover OAuth2 configuration values. This MUST be in the form of a URL.
...
Field Name | Type | Applies To | Description
---|:---:|---|---
<a name="oauthFlowAuthorizationUrl"></a>authorizationUrl | `string` | `oauth2` (`"implicit"`, `"authorizationCode"`) | **必須**。 The authorization URL to be used for this flow. This MUST be in the form of a URL.
<a name="oauthFlowTokenUrl"></a>tokenUrl | `string` | `oauth2` (`"password"`, `"clientCredentials"`, `"authorizationCode"`) | **必須**。 The token URL to be used for this flow. This MUST be in the form of a URL.
...
<a name="oauthFlowScopes"></a>scopes | Map[`string`, `string`] | `oauth2` | **必須**。 The available scopes for the OAuth2 security scheme. A map between the scope name and a short description for it.

このオブジェクトは [Specification Extensions](#specificationExtensions) によって拡張できる(MAY)。

##### OAuth Flow オブジェクトの例s
